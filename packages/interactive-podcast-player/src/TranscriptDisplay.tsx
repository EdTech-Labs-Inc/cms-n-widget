...
function TranscriptDisplay({ segments, currentTime, isPlaying, onInteractiveAnswer, completedPrompts, isPausedForInteraction }: TranscriptDisplayProps) {
  ...
  // Create audio elements for sound feedback - using relative paths from assets
  const duolingoCorrect = new URL('../assets/duolingo-correct.mp3', import.meta.url).href
  const duolingoIncorrect = new URL('../assets/duolingo-incorrect.mp3', import.meta.url).href
  const correctAudio = useMemo(() => new Audio(duolingoCorrect), [duolingoCorrect])
  const incorrectAudio = useMemo(() => new Audio(duolingoIncorrect), [duolingoIncorrect])
