...
/**
 * POST /api/org/[orgSlug]/submissions/[submissionId]/video/[videoId]/regenerate-thumbnail
 * Regenerate the thumbnail for a video output with custom prompt
 */
export async function POST(
  ...
  try {
    ...
    const result = await SubmissionsController.regenerateVideoThumbnail(
      params.submissionId,
      params.videoId,
      customPrompt
    );
    ...
    console.error('Regenerate Video Thumbnail Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to regenerate video thumbnail',
      },
      { status: 500 }
    );
  }
