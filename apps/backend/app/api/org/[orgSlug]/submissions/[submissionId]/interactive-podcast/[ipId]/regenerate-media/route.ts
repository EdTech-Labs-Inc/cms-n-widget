...
/**
 * POST /api/org/[orgSlug]/submissions/[submissionId]/interactive-podcast/[ipId]/regenerate-media
 * Regenerate the media for an interactive podcast output
 */
export async function POST(
  ...
  try {
    ...
    // Regenerate interactive podcast media
    const result = await SubmissionsController.regenerateInteractivePodcastMedia(
      params.submissionId,
      params.ipId
    );
    ...
    console.error('Regenerate Interactive Podcast Media Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to regenerate interactive podcast media',
      },
      { status: 500 }
    );
  }
