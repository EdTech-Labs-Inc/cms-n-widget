...
/**
 * POST /api/org/[orgSlug]/submissions/[submissionId]/interactive-podcast/[ipId]/regenerate-script
 * Regenerate the script for an interactive podcast output with optional custom prompt
 */
export async function POST(
  ...
  try {
    ...
    // Regenerate interactive podcast script
    const result = await SubmissionsController.regenerateInteractivePodcastScript(
      params.submissionId,
      params.ipId,
      customPrompt
    );
    ...
    console.error('Regenerate Interactive Podcast Script Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to regenerate interactive podcast script',
      },
      { status: 500 }
    );
  }
