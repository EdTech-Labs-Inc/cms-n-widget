...
/**
 * DELETE /api/org/[orgSlug]/submissions/[submissionId]/quiz/[quizId]/tags/[tagId]
 * Remove a tag from a quiz output
 */
export async function DELETE(
  request: NextRequest,
  props: { params: Promise<{ orgSlug: string; submissionId: string; quizId: string; tagId: string }> }
) {
  ...
  try {
    ...
    // Remove tag from quiz output
    const result = await SubmissionsController.removeQuizTag(params.quizId, params.tagId);
    ...
    console.error('Remove Quiz Tag Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to remove tag from quiz',
      },
      { status: 500 }
    );
  }
