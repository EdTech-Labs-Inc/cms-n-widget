...
/**
 * DELETE /api/org/[orgSlug]/submissions/[submissionId]/video/[videoId]/tags/[tagId]
 * Remove a tag from a video output
 */
export async function DELETE(
  request: NextRequest,
  props: { params: Promise<{ orgSlug: string; submissionId: string; videoId: string; tagId: string }> }
) {
  ...
  try {
    ...
    // Remove tag from video output
    const result = await SubmissionsController.removeVideoTag(params.videoId, params.tagId);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to remove tag from video',
      },
      { status: 500 }
    );
  }
