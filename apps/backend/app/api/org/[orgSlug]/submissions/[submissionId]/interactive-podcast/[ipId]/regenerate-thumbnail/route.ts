...
/**
 * POST /api/org/[orgSlug]/submissions/[submissionId]/interactive-podcast/[ipId]/regenerate-thumbnail
 * Regenerate the thumbnail for an interactive podcast output with custom prompt
 */
export async function POST(
  ...
  try {
    ...
    // Regenerate interactive podcast thumbnail
    const result = await SubmissionsController.regenerateInteractivePodcastThumbnail(
      params.submissionId,
      params.ipId,
      customPrompt
    );
    ...
    console.error('Regenerate Interactive Podcast Thumbnail Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to regenerate interactive podcast thumbnail',
      },
      { status: 500 }
    );
  }
