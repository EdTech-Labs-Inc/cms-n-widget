...
/**
 * POST /api/org/[orgSlug]/submissions/[submissionId]/video/[videoId]/regenerate-script
 * Regenerate the script for a video output with optional custom prompt
 */
export async function POST(
  ...
  try {
    ...
    const result = await SubmissionsController.regenerateVideoScript(
      params.submissionId,
      params.videoId,
      customPrompt
    );
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to regenerate video script',
      },
      { status: 500 }
    );
  }
