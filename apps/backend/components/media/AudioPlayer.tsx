...
import { Loader2, AlertCircle, Clock, Play, Pause, Volume2, VolumeX } from 'lucide-react';
import type { AudioOutput } from '@/lib/api/types';

interface AudioPlayerProps {
  output: AudioOutput;
}

export function AudioPlayer({ output }: AudioPlayerProps) {
    ...
    return (
      <div className="flex items-center justify-center py-8 gap-3">
        ...
        <div className="text-text-secondary">
          {output.status === 'PENDING' ? 'Queued...' : 'Generating audio...'}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white-10 border-2 border-white-20 rounded-3xl p-4">
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={togglePlay}
            className="w-12 h-12 bg-gradient-pink rounded-full flex items-center justify-center hover:shadow-lg transition-all shrink-0"
          >
            {isPlaying ? (
          ...
          <div className="flex-1 space-y-2">
            ...
            >
              ...
              />
              {/* Playback progress (pink gradient for audio) */}
              <div
                className="absolute inset-y-0 left-0 bg-gradient-pink rounded-full transition-all"
                style={{ width: `${(currentTime / duration) * 100}%` }}
              />
              {/* Progress indicator */}
              <div
                className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-pink-accent rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
