...
interface InteractivePodcastAudioPlayerProps {
  output: InteractivePodcastOutput;
}

export function InteractivePodcastAudioPlayer({ output }: InteractivePodcastAudioPlayerProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white-10 border-2 border-white-20 rounded-3xl p-4">
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={togglePlay}
            className="w-12 h-12 bg-gradient-blue rounded-full flex items-center justify-center hover:shadow-lg transition-all shrink-0"
          >
            {isPlaying ? (
          ...
          <div className="flex-1 space-y-2">
            ...
            >
              ...
              />
              {/* Playback progress (blue gradient for interactive podcast) */}
              <div
                className="absolute inset-y-0 left-0 bg-gradient-blue rounded-full transition-all"
                style={{ width: `${(currentTime / duration) * 100}%` }}
              />
              {/* Progress indicator */}
              <div
                className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-accent rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {output.duration && (
        <p className="text-text-muted text-sm">
          ...
          {(output.duration % 60).toString().padStart(2, '0')}
        </p>
      )}
    </div>
  );
